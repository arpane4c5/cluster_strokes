...
def stroke_filter(vid_pts, str_tuples, clip_length):
    ...
    for vid_pt in vid_pts:
        if len(str_tuples) > 0 and vid_pt >= end:
            ...
            stroke_pts = []
        if vid_pt >= start and vid_pt < end:
            stroke_pts.append(vid_pt)
...
class VideoClips(object):
    ...
    """
    def __init__(self, video_paths, strokes_list, clip_length_in_frames=16, 
                 frames_between_clips=1, frame_rate=None, _precomputed_metadata=None):
