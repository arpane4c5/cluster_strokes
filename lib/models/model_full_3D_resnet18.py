...
from convrnn_cpu import ConvGRU
...
class DPC_RNN(nn.Module):
    ...
    def forward(self, block):
        ...
        #score = mseloss(pred, feature_inf)
        ...
        if self.mask is None: # only compute mask once
            ...
            if self.device == 'cpu':
                mask = torch.zeros((B, self.pred_step, self.last_size**2, B, N, self.last_size**2), dtype=torch.int8, requires_grad=False).detach()
            else:
...
if __name__=='__main__':
    model = DPC_RNN(sample_size=128, num_seq=2, 
                    seq_len=16, pred_step=1 ,network='full3Dresnet18')
    x = torch.randn(1, 2, 3, 16, 112, 112)
    pred, gt = model(x)
