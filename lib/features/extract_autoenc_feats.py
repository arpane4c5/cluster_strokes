...
from datasets.dataset_8classes import CricketStrokesBBoxesDataset
import pickle
...
def extract_2DCNN_bboxes_feats(DATASET, LABELS, BAT_LABELS, feats, snames, 
                               CLASS_IDS, BATCH_SIZE, 
                               partition='all', nstrokes=-1):
    '''
    Extract features extracted from bboxes of batsman poses.
    ...
    if partition == 'all':
        ...
        partition_lst.extend(val_lst)
        partition_lst.extend(test_lst)
        bbox_feats = [os.path.join(BAT_LABELS, f) for f in feats]
        bbox_snames = [os.path.join(BAT_LABELS, f) for f in snames]
    elif partition == 'train':
        partition_lst = train_lst
        bbox_feats = [os.path.join(BAT_LABELS, feats[0])]
        bbox_snames = [os.path.join(BAT_LABELS, snames[0])]
    elif partition == 'val':
        partition_lst = val_lst
        bbox_feats = [os.path.join(BAT_LABELS, feats[1])]
        bbox_snames = [os.path.join(BAT_LABELS, snames[1])]
    elif partition == 'test':
        partition_lst = test_lst
        bbox_feats = [os.path.join(BAT_LABELS, feats[2])]
        bbox_snames = [os.path.join(BAT_LABELS, snames[2])]
    ...
    clip_transform = transforms.Compose([transforms.ToPILImage(),
                                         ...
                                         transforms.Normalize(mean=[0.485, 0.456, 0.406], 
                                                              std=[0.229, 0.224, 0.225]),]) 
    # For using Frame level transform, the framewiseTransform flag turned on
    part_dataset = CricketStrokesBBoxesDataset(partition_lst, DATASET, LABELS, bbox_feats,
                                               bbox_snames, CLASS_IDS, frames_per_clip=1, 
