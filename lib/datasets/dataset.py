...
#        end_pts = clip_pts[-1].item()
        ...
        return src_sequence, video_path, stroke_tuple, tar_sequence, label
    ...
    def generateSequencePairs(self, future_step = 1):
        ...
        for i in range(0, len(strokes), future_step):
            ...
            if i==0:
                continue
            if s == prev:
                pairs.append([vid_clip_idx[i - future_step], vid_clip_idx[i]])
            else:
                prev = s
        return pairs
...
class StrokeMultiFeatureSequenceDataset(VisionDataset):
    """
    `Cricket Multi Feature Sequence Dataset for strokes dataset.
    ...
    def __init__(self, feat_path, videos_list, dataset_path, strokes_dir, class_ids_path, 
                 ...
                 _precomputed_metadata=None, num_workers=1, _video_width=0, 
                 _video_height=0, _video_min_dimension=0, _audio_samples=0):
        super(StrokeMultiFeatureSequenceDataset, self).__init__(dataset_path)
...
#        end_pts = clip_pts[-1].item()
        ...
        src_seqs = []
        for vid_ft in vid_feats:
            src_seqs.append(vid_ft[seq_start_idx:(seq_start_idx+seq_len), :])
        nrows = sum([s.shape[0] for s in src_seqs])
        ncols = sum([s.shape[1] for s in src_seqs])
        sequence = np.zeros((nrows, ncols))
        for i, src_seq in enumerate(src_seqs):
            rows = np.array(list(range(i, nrows, len(vid_feats))))
            cols = np.where(src_seq == 1)[1]
            cols = cols + (i * src_seq.shape[1])
            sequence[rows, cols] = 1
...
class CricketStrokesFlowDataset(VisionDataset):
    """
    `Cricket Dataset for strokes dataset.
    ...
    def __init__(self, videos_list, dataset_path, strokes_dir, class_ids_path, 
                 ...
                 _precomputed_metadata=None, num_workers=1, _video_width=0, 
                 _video_height=0, _video_min_dimension=0, _audio_samples=0):
        super(CricketStrokesFlowDataset, self).__init__(dataset_path)
    ...
    def get_center_crop(self, imgs, size=224):
        t, h, w, c = imgs.shape
        th, tw = size, size
    ...
    def __getitem__(self, idx):
        video, vid_path, stroke, audio, info, video_idx = self.video_clips.get_clip(idx)
        # video is (Seq, H, W, Ch) for a sample in a batch, with dtype uint8

#        flow_vid = self.get_center_crop(video, 224)
        flow_vid = video
        prev_ = cv2.cvtColor(flow_vid[0].numpy(), cv2.COLOR_BGR2GRAY)
#        prev_ = cv2.resize(prev_, (0,0), fx=0.5, fy=0.5)
        flow_bgr = []
        for i in range(1, flow_vid.shape[0]):
            next_ = cv2.cvtColor(flow_vid[i].numpy(), cv2.COLOR_BGR2GRAY)
#            next_ = cv2.resize(next_, (0,0), fx=0.5, fy=0.5)
            flow = cv2.calcOpticalFlowFarneback(prev_, next_, None, 0.5, 3, 15, 3, 5, 1.2, 0)
            flow_bgr.append(draw_flow_bgr(flow))
#            flow_bgr.append(flow)
            prev_ = next_
#            mag, ang = mag, ang = cv2.cartToPolar(flow[...,0], flow[...,1])       
        flow = torch.tensor(flow_bgr)
...
#            video = np.squeeze(video, axis=0)
        ...
        return video, vid_path, stroke, flow, label
    
# Convert a flow matrix to the HSV and then RGB image space
def draw_flow_bgr(flow):
    ht, wd, _ = flow.shape
    hsv = np.zeros((ht, wd, 3), dtype=np.uint8)
