...
def optimize_subspace(trajectories, row_names, mean_feats, log_dir, 
                      nclasses=3, write_strokes=False):
    X = pd.DataFrame(trajectories, index=row_names)
    aff_mat_filepath = os.path.join(log_dir, "aff_mat_"+similarity+".npy")
...
#    strokes_name_id.extend(strokes_name_id_test)
    ...
    elif clustering is 'optimize':
        acc = optimize_subspace(trajectories, strokes_name_id, all_feats, log_dir, 
                                nclasses,
                                write_strokes)
...
#    base_path = "/home/arpan/VisionWorkspace/Cricket/StrokeAttention/logs"
    ...
    clustering = 'optimize'  # 'kmeans' , 'spectral'
